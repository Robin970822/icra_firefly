...
def ssd_bboxes_decode(feat_localizations,
    ...
    return bboxes
#
def ssd_bboxes_decode_p(feat_localizations,
                      anchor_bboxes,
                      prior_scaling=[0.1, 0.1, 0.2, 0.2],
                      isF=True):
    """Compute the relative bounding boxes from the layer features and
    ...
    l_shape = feat_localizations.shape
    if isF:
        feat_localizations = tf.reshape(feat_localizations,
                                    (-1,4, 4))
    else:
        feat_localizations = tf.reshape(feat_localizations,
                                        (-1, 6, 4))
    ...
    xref = np.reshape(xref, [-1, 1])
    yref = np.reshape(yref, [-1, 1])
    # yref=tf.Variable(yref)
    # xref = tf.Variable(xref)
    # href = tf.Variable(href)
    # wref = tf.Variable(wref)
    # yref = tf.Variable(,)
    # xref = tf.cast(xref)
    # href = tf.cast(href)
    # wref = tf.cast(wref)
    ...
    # bboxes: ymin, xmin, xmax, ymax.
    bboxes=tf.stack([cy - h / 2.,cx - w / 2.,cy + h / 2.,cx + w / 2.],axis=2)
    # bboxes[:, :, 0] = cy - h / 2.
    # bboxes[:, :, 1] = cx - w / 2.
    # bboxes[:, :, 2] = cy + h / 2.
    # bboxes[:, :, 3] = cx + w / 2.
...
def ssd_bboxes_select_layer_p(predictions_layer,
                            localizations_layer,
                            anchors_layer,
                            select_threshold=0.5,
                            img_shape=(300, 300),
                            num_classes=21,
                            decode=True,isF=True):
    """Extract classes, scores and bounding boxes from features in one layer.
    ...
    # First decode localizations features if necessary.
    if decode:
        localizations_layer = ssd_bboxes_decode_p(localizations_layer, anchors_layer,isF=isF)
    ...
    return predictions_layer, localizations_layer
...
def ssd_bboxes_select_p(predictions_net,
                      localizations_net,
                      anchors_net,
                      select_threshold=0.5,
                      img_shape=(300, 300),
    ...
    for i in range(len(predictions_net)):
        if i==1 or i==2 or i==3:
            predictions, localizations = ssd_bboxes_select_layer_p(
            predictions_net[i], localizations_net[i], anchors_net[i],
            select_threshold, img_shape, num_classes, decode,isF=False)
        else:
            predictions, localizations = ssd_bboxes_select_layer_p(
            predictions_net[i], localizations_net[i], anchors_net[i],
            select_threshold, img_shape, num_classes, decode)
        l_predictions.append(predictions)
        l_localizations.append(localizations)
        # Debug information.
        # l_layers.append(i)
        # l_idxes.append((i, idxes))

    predictions = tf.concat(l_predictions, 1)
    localizations = tf.concat(l_localizations, 1)
    return predictions, localizations
